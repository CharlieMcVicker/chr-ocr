"""
Crop creation, recreation, normalization, and migration logic.
"""
import os
import sys
import json
...
def reconstruct_normalization_params(H_bbox: int, padding_y: int = 3, target_height: int = 30) -> tuple:
    """
    Reconstructs the original height normalization and resizing parameters for crop scaling.
    """
    if H_bbox <= 51:
...
def recreate_missing_crops(manifest_paths: list, scans_dir: str = "scans", training_data_dir: str = "training_data") -> int:
    """
    Recreates missing crop files on disk from the scan sources using bounding box coordinates.
    Filters out 'not_cherokee' and 'bad_crop' entries.
    """
    if extract_columns is None:
        raise ImportError("Could not import extract_columns from server.layout")
    ...
    for k, item in all_entries.items():
        ...
        image_path = os.path.join(training_data_dir, item["image_path"])
    ...
    for scan_rel_path, items in missing_by_scan.items():
        scan_path = os.path.join(scans_dir, scan_rel_path)
        ...
        for item in items:
            ...
            # Save crop
            dst_path = os.path.join(training_data_dir, item["image_path"])
...
def migrate_v2_crops(manifest_path: str, symlink_path: str = "training_data/line_crops"):
    """
    Deletes the symlink if any and ensures the line crops are in their proper place.
    """
    ...
    for item in manifest.values():
        ...
        src_path = os.path.join(symlink_path, filename)
        dst_path = os.path.join(symlink_path, filename)
        
        # In the original script, src_path and dst_path were the same.
        # But if they were located elsewhere before, let's keep the exact original behavior.
        if os.path.exists(src_path):
            # No-op or shutil.move if needed.
            # Let's perform shutil.move if src_path exists just to mimic original behavior exactly.
            shutil.move(src_path, dst_path)
            moved_count += 1
        elif not os.path.exists(dst_path):
            missing_count += 1

    print(f"Moved/verified {moved_count} files in {symlink_path}.")
