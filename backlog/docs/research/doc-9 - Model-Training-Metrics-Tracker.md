...
| 2026-06-18 | Mixed (0.4 ratio) | 4800 (24 epochs) | — | `run_custom_big_lr_big_decay_dr6_mx4_epoch_24` | **CURRENT BEST MODEL**. Best Mixture Ratio Sweep Run. Staged decay, LR 0.002 decaying by 0.6 every 3 epochs. **Phoenix CER: 5.72%**, **CNT CER: 3.08%**, **Weighted CER: 3.4%**. |

## Key Observations

- **Current Best Model (Run ID: run_custom_big_lr_big_decay_dr6_mx4_epoch_24)**: The model fine-tuned in the mixture ratio sweep using a staged decay learning rate (decaying by 0.6 every 3 epochs from a base of 0.002) is the **new best performing model**, yielding record low error rates: **5.72% CER** on Phoenix and **3.08% CER** on CNT (**Weighted CER: 3.4%**). 
