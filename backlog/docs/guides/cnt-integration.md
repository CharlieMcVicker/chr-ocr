...
Generated by `scripts/align_verses.py`, this file details the final words assigned to each line crop, recording both the raw OCR outputs (stock and fine-tuned model versions) and the aligned ground-truth text.
...
The alignment is implemented as a **Dynamic Programming (DP)** optimization in `scripts/align_verses.py` under the function `align_words_to_lines`. 
...
The system resolves this via an automated **Iterative Feedback Loop** across three scripts: `find_missing_spaces.py`, `apply_space_corrections.py`, and the main orchestrator `align_verses.py`.

```mermaid
sequenceDiagram
    participant Align as align_verses.py
    participant Manifest as aligned_manifest.json
    participant Find as find_missing_spaces.py
    participant Apply as apply_space_corrections.py
...
When `apply_space_corrections.py` executes, it filters out these exceptions and modifies both `segment_map.json` and `metadata.json` to insert spaces at the flagged indices. If any changes are written, `align_verses.py` catches the file modification and triggers a secondary alignment pass to output a clean, final `aligned_manifest.json`.
...
```bash
uv run scripts/align_verses.py \
...
```bash
uv run scripts/find_missing_spaces.py \
...
```bash
uv run scripts/apply_space_corrections.py \
  --book-dir training_data/cnt/book_01 \
